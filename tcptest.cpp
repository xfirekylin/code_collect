...
static void mmi_hex_to_ascii(unsigned char *src, unsigned char *dest, unsigned int src_len)
{
    /*----------------------------------------------------------------*/
    /* Local Variables                                                */
    /*----------------------------------------------------------------*/
...
int md5_file(char *filename, unsigned char *md5_result)
{
    MD5_CTX ctx;
    ...
    while (total>0)
    {
       int readlen = total >= BUFLEN ? BUFLEN:total;
    ...
    mmi_hex_to_ascii(ctx.digest, md5_result, 16);
    printf ("file %s md5:%s\n",filename,md5_result);
...
{  
    ...
    if (1 == argc)
    {
      strcpy(file_name,SENT_FILE_NAME);
    }
    else if (2 == argc)
    {
    ...
    {
      printf("parameter error:tcptest filename or tcptest");
      return 0;
    }
    ...
    md5_file(file_name, (unsigned char*)md5_str);
    ...
    while (1)
    {
        ...
        {  
            ...
            receive_result = recv(clientsoc, buf, BUFLEN, 0);
            if(receive_result <= 0)   
            {  
                //some error occur  
            ...
            else if (0 == strncmp(buf, "DL_DATA", strlen("DL_DATA")))
            {
                ...
                send_result = send( clientsoc, buf, 4, 0 );
                 ...
                 {
                    int readlen = total >= BUFLEN ? BUFLEN:total;
                    send_result = fread(buf, readlen, 1, fp);
                    send_result = send( clientsoc, buf, readlen, 0 );
                    if (SOCKET_ERROR == send_result)
                    {
                        printf("Close connection\n");  
                        closesocket(clientsoc);  
            ...
            else if (0 == strncmp(buf, "DL_MID5", strlen("DL_MID5")))
            {
                int len = 36;
                memset(buf, 0, BUFLEN);
                memcpy(buf,&len,4);
